 ...
 * @param tradingDay - Date object representing the trading day (will use midnight of this day)
...
export function getMarketHoursBounds(tradingDay: Date): MarketHoursBounds {
  ...
  const dayET = new Date(tradingDay.toLocaleString('en-US', { timeZone: 'America/New_York' }));
  dayET.setHours(0, 0, 0, 0);
  
  const extendedOpen = new Date(dayET);
  extendedOpen.setHours(EXTENDED_MARKET_HOURS.openHour, EXTENDED_MARKET_HOURS.openMinute, 0, 0);
  
  const regularOpen = new Date(dayET);
  regularOpen.setHours(EXTENDED_MARKET_HOURS.regularOpenHour, EXTENDED_MARKET_HOURS.regularOpenMinute, 0, 0);
  
  const regularClose = new Date(dayET);
  regularClose.setHours(EXTENDED_MARKET_HOURS.regularCloseHour, EXTENDED_MARKET_HOURS.regularCloseMinute, 0, 0);
  
  const extendedClose = new Date(dayET);
  extendedClose.setHours(EXTENDED_MARKET_HOURS.closeHour, EXTENDED_MARKET_HOURS.closeMinute, 0, 0);
  
  return {
    extendedOpen: extendedOpen.getTime(),
    regularOpen: regularOpen.getTime(),
    regularClose: regularClose.getTime(),
    extendedClose: extendedClose.getTime(),
  };
}
...
export function getTradingDayFromData(data: UnifiedChartPoint[] | { timestamp: number }[]): Date {
  ...
  const tradingDayET = new Date(firstDate.toLocaleString('en-US', { timeZone: 'America/New_York' }));
